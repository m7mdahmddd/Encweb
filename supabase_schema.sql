...
CREATE OR REPLACE FUNCTION public.delete_user_account(user_id_to_delete uuid DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  target_id uuid;
BEGIN
  target_id := COALESCE(user_id_to_delete, auth.uid());
  
  IF target_id IS NULL THEN
    RAISE EXCEPTION 'No user ID provided or user not authenticated';
  END IF;

  DELETE FROM public.friends WHERE user_id = target_id OR friend_id = target_id;
  DELETE FROM public.secret_messages WHERE sender_id = target_id;
